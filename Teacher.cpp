...
bool Teacher::validateTeacherID(const string& id) const {
    return !id.empty() && all_of(id.begin(), id.end(), [](char c) { return isalnum(c); });
}

Teacher::Teacher() : Person(), teacherID(""), department(""), academicDegree(AcademicDegree::BACHELOR) {
...
Teacher::Teacher(const string& name, const string& lastName,
    ...
    : Person(name, lastName, email), teacherID(teacherID), department(department), academicDegree(degree) {
    initializeUkrainianSupport();
    if (!validateTeacherID(teacherID)) throw invalid_argument("Некоректний ID викладача: " + teacherID);
}

Teacher::Teacher(const Teacher& other)
    : Person(other), teacherID(other.teacherID), department(other.department),
      academicDegree(other.academicDegree) {
}

Teacher::Teacher(Teacher&& other) noexcept
    : Person(move(other)), teacherID(move(other.teacherID)), department(move(other.department)),
...
void Teacher::setTeacherID(const string& id) {
    if (!validateTeacherID(id)) throw invalid_argument("Некоректний ID викладача: " + id);
    teacherID = id;
}

void Teacher::setDepartment(const string& dep) {
    if (dep.empty()) throw invalid_argument("Кафедра не може бути порожньою");
    department = dep;
}
